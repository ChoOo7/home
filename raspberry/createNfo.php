...
{	
	...
	public function getIMDbIdFromSearch($title, $engine = "google"){
		switch ($engine) {
...
foreach($files as $file)
{
    ...
    $nfoPath = str_replace('.'.$ext, '.nfo', $fullPath);
    ...
    $cleanedFileName = $file;
    
    $ignoreStrings = array('.avi', '.mkv', '.mp4', 'www.CpasBien.cm', 'TRUEFRENCH', 'french', 'vostfr', 'multi', '{','[',']', '}', '.', '1080p', '720p', 'webrip', 'dvdrip', 'xvid', 'FUNKKY', 'h264', 'hdtv', 'blueray', 'bdrip', '-', 'bluray', 'lost', 'brrip', 'x264', 'ACOOL', ' HD ', ' WEB ', '(', ')', 'mkv', 'ac3' );
    foreach($ignoreStrings as $ignoreString)
    {
        $cleanedFileName = str_ireplace($ignoreString, ' ', $cleanedFileName);
    }
    $cleanedFileName = trim($cleanedFileName);
    while(strpos($cleanedFileName, '  ') !== false)
    {
    ...
    $im = new Imdb();
    $id = $im->getIMDbIdFromSearch($cleanedFileName);
    echo "\n\t=>".$id;
    file_put_contents($nfoPath, '<movie>
        <title>'.$cleanedFileName.'</title>
</movie>
